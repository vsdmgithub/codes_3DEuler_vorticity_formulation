...
! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
  ...
  DOUBLE PRECISION,DIMENSION(:),ALLOCATABLE::loc_vx_dot_moment,bck_vx_dot_moment
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    SUBROUTINE allocate_bck_strain_opr
    ! INFO - START  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    ! ------------
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    !  ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    WRITE(7778,f_d8p4,ADVANCE   ='no')  time_now
    WRITE(7779,f_d8p4,ADVANCE   ='no')  time_now

    DO m_ind = m_ind_min, m_ind_max - 1

      WRITE(7778,f_d32p17,ADVANCE ='no') bck_vx_dot_moment( m_ind )
      WRITE(7779,f_d32p17,ADVANCE ='no') loc_vx_dot_moment( m_ind )

    END DO

    WRITE(7778,f_d32p17,ADVANCE ='yes') bck_vx_dot_moment( m_ind )
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    DO i_y = 0, N - 1
    DO i_z = 0, N - 1
         WRITE(788,'(F32.17)',ADVANCE='yes') vx_stretching(i_x,i_y,i_z)
    END DO
    END DO
  ...
  ! INFO - END <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    DO i_y = 0, N - 1
    DO i_z = 0, N - 1
         WRITE(789,'(F32.17)',ADVANCE='yes') vx_stretching(i_x,i_y,i_z) - bck_vx_stretching(i_x,i_y,i_z)
    END DO
    END DO
