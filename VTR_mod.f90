...
module paraview
  ...
  subroutine VTR_open_file(prefix, proc_rank, num_procs, restart, fd)
    implicit none
    ...
    if (fd%unit == 0 .and. file_opened) then
      ...
      if ( present(proc_rank) ) then
        ...
        write(rank,'(i8)') iproc
        f=trim(fd%prefix)//"_"//trim(adjustl(rank))//"_"//trim(adjustl(snapshot))//".vtr"

      else

        f=trim(fd%prefix)//"_"//trim(adjustl(snapshot))//".vtr"
